...
Ext.define("TSApp", {
    ...
    _queryAndDisplayGrid: function(){
        ...
        var model_name = 'Task',
            field_names = ['ObjectID','FormattedID','Name','Project','State','Owner','WorkProduct','ToDo','Estimate','Actuals','Iteration','UserIterationCapacities','DisplayName',"FirstName",'LastName'],
                    ...
                    if(teamExists.length < 1){
                        ...
                        Ext.Array.each(me.uic_hash[task.get('Project').Name].Users,function(user){
                            if(user.User == userName){
                                user.children.push(me._getLeafNode(task));
                                user.Capacity = capacity;
                                user.Estimate = task.get('Estimate');
                                user.ToDo = task.get('ToDo');
                                user.Actuals = task.get('Actuals');
                                user.PercentageUsedEstimate = me._getPercentage(task.get('Estimate'),capacity);
                                user.PercentageUsedToDo = me._getPercentage(task.get('ToDo'),capacity);
                                user.PercentageUsedActuals = me._getPercentage(task.get('Actuals'),capacity);
                            }
                        })
                        task = {
